...
<head>
    ...
    <style>
        ...
        .add-form-container {
            ...
            opacity: 1;
            overflow: hidden;
            transition: max-height 0.5s ease-in-out, opacity 0.3s ease-in-out, padding 0.3s ease-out, margin 0.3s ease-out;
            padding: 1.5rem; 
        ...
        .category-row:hover { background-color: #f9fafb; } 
...
<body class="bg-gray-100 flex antialiased">
    ...
    <main class="ml-64 w-full p-6 md:p-8 lg:p-10">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <button id="toggleAddFormBtn" type="button" class="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    ...
                    <span id="toggleAddFormBtnText">Ajouter Catégorie</span> 
                </button>
            </div>
            ...
            <div id="addCategoryFormContainer" class="add-form-container {{ $errors->any() ? '' : 'hidden' }} bg-white shadow rounded-lg border border-gray-200">
                 ...
                 <form id="add-category-form" action="{{ route('categories.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="add-name" value="{{ old('name') }}" required
                               class="block w-full shadow-sm sm:text-sm rounded-md border-gray-300 focus:ring-blue-500 focus:border-blue-500 @error('name') border-red-500 @enderror"
                    ...
                    <div class="flex justify-end space-x-3">
                        ...
                        <button type="button" id="cancelAddBtn" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            Annuler
                        </button>
                 ...
                 @isset($categories)
                     @if($categories->count() > 0)
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <thead class="bg-gray-50"> <tr> <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nom</th> <th scope="col" class="relative px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th> </tr> </thead>
                                ...
                                @foreach ($categories as $category)
                                    ...
                                    <tr class="category-row" x-data="{ editing: false, categoryName: '{{ addslashes($category->name) }}' }">
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <div x-show="!editing" @dblclick="editing = true; $nextTick(() => $refs.editInput{{ $category->id }}.select())" class="text-sm font-medium text-gray-800 truncate cursor-pointer py-1" title="Double-cliquer pour modifier"> {{ $category->name }} </div>
                                            <form x-show="editing" x-cloak @submit.prevent="$el.submit()" @keydown.escape.window="editing = false" method="POST" action="{{ route('categories.update', $category->id) }}" class="relative w-full"> @csrf @method('PUT') <label for="edit-name-{{ $category->id }}" class="sr-only">Modifier nom</label> <input type="text" name="name" id="edit-name-{{ $category->id }}" x-ref="editInput{{ $category->id }}" x-model="categoryName" class="block w-full shadow-sm sm:text-sm rounded-md border-gray-300 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 pr-16 py-1 @error('name') border-red-500 @enderror" required> <div class="absolute inset-y-0 right-0 flex items-center pr-1 space-x-1"> <button type="button" @click="editing = false" class="p-1 text-gray-400 hover:text-gray-600 rounded-full focus:outline-none focus:ring-2 focus:ring-gray-400" title="Annuler"> <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /> </svg> </button> <button type="submit" class="p-1 text-blue-600 hover:text-blue-800 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500" title="Enregistrer"> <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" /> </svg> </button> </div> @if($errors->has('name') && old('_method') === 'PUT') <p class="mt-1 text-xs text-red-600">{{ $errors->first('name') }}</p> @endif </form>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="action-buttons flex items-center justify-end space-x-2" :class="{ 'opacity-100': editing }">
                                                <button x-show="!editing" @click="editing = true; $nextTick(() => $refs.editInput{{ $category->id }}.select())" type="button" title="Modifier" class="p-1 text-blue-600 hover:text-blue-800 focus:outline-none"> <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" /> </svg> </button>
                                                <form x-show="!editing" method="POST" action="{{ route('categories.destroy', $category->id) }}" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer la catégorie \'{{ addslashes($category->name) }}\' ?\nCette action ne peut pas être annulée.')"> @csrf @method('DELETE') <button type="submit" title="Supprimer" class="p-1 text-red-600 hover:text-red-800 focus:outline-none"> <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /> </svg> </button> </form>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const addFormContainer = document.getElementById('addCategoryFormContainer');
            const cancelBtn = document.getElementById('cancelAddBtn');
            const toggleBtnText = document.getElementById('toggleAddFormBtnText');
            const addForm = document.getElementById('add-category-form'); 

            
            function updateToggleButtonText() {
                if (addFormContainer.classList.contains('hidden')) {
                    toggleBtnText.textContent = 'Ajouter Catégorie';
                } else {
                    toggleBtnText.textContent = 'Masquer Formulaire';
                }
            }
